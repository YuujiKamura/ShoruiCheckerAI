...
根拠となる証票の写し、あるいは図面と集計表の整合性を確認すること。  
つぎに単体の書面として期待される書式からの逸脱に気づくこと。  
この2つをAIで補助する。

細部まで見たか思い出せない書類は、高確率で何処かが間違っている。  
そして大抵の場合、そこまで細かく見ていない。
...
3. **次回チェック時に参照** - 解析対象の**親フォルダ直下**の `.guidelines.json` を参照してチェック（1フォルダ=1ファイル）
   - **サブフォルダには作成しない**（再帰探索もしない）
   - **書式別ガイドラインは同じファイル内**の `categories` に格納（ファイル分割はしない）
   - 解析時は**該当カテゴリのみを抽出して参照**（全件読み込みはしない）

## 使い方

（最終更新: 2026-02-01 / commit: 13b478f）
...
再解析すると **PDF内の埋め込みは上書き**され、過去の解析結果は残らない。  
※ 解析時にカスタム指示が空の場合、**前回の指示が残る**ことがある。  
履歴は別途 `history` に保存される。

### ガイドライン

`.guidelines.json` は**1フォルダにつき1ファイル**。  
内部は「共通（common）」と「書類タイプ別（categories）」で管理する。  
